package auth

import (
	"net/http"
	"testing"
	"time"

	"github.com/google/uuid"
)

func TestCheckPasswordHash(t *testing.T) {
	// First, we need to create some hashed passwords for testing
	password1 := "correctPassword123!"
	password2 := "anotherPassword456!"
	hash1, _ := HashPassword(password1)
	hash2, _ := HashPassword(password2)

	tests := []struct {
		name          string
		password      string
		hash          string
		wantErr       bool
		matchPassword bool
	}{
		{
			name:          "Correct password",
			password:      password1,
			hash:          hash1,
			wantErr:       false,
			matchPassword: true,
		},
		{
			name:          "Incorrect password",
			password:      "wrongPassword",
			hash:          hash1,
			wantErr:       false,
			matchPassword: false,
		},
		{
			name:          "Password doesn't match different hash",
			password:      password1,
			hash:          hash2,
			wantErr:       false,
			matchPassword: false,
		},
		{
			name:          "Empty password",
			password:      "",
			hash:          hash1,
			wantErr:       false,
			matchPassword: false,
		},
		{
			name:          "Invalid hash",
			password:      password1,
			hash:          "invalidhash",
			wantErr:       true,
			matchPassword: false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			match, err := VerifyPassword(tt.password, tt.hash)
			if (err != nil) != tt.wantErr {
				t.Errorf("VerifyPassword() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !tt.wantErr && match != tt.matchPassword {
				t.Errorf("VerifyPassword() expects %v, got %v", tt.matchPassword, match)
			}
		})
	}
}

func TestValidateJWT(t *testing.T) {
	userID := uuid.New()
	validToken, _ := MakeJWT(userID, "secret", time.Hour)

	tests := []struct {
		name        string
		tokenString string
		tokenSecret string
		wantUserID  uuid.UUID
		wantErr     bool
	}{
		{
			name:        "Valid token",
			tokenString: validToken,
			tokenSecret: "secret",
			wantUserID:  userID,
			wantErr:     false,
		},
		{
			name:        "Invalid token",
			tokenString: "invalid.token.string",
			tokenSecret: "secret",
			wantUserID:  uuid.Nil,
			wantErr:     true,
		},
		{
			name:        "Wrong secret",
			tokenString: validToken,
			tokenSecret: "wrong_secret",
			wantUserID:  uuid.Nil,
			wantErr:     true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotUserID, err := ValidateJWT(tt.tokenString, tt.tokenSecret)
			if (err != nil) != tt.wantErr {
				t.Errorf("ValidateJWT() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotUserID != tt.wantUserID {
				t.Errorf("ValidateJWT() gotUserID = %v, want %v", gotUserID, tt.wantUserID)
			}
		})
	}
}

func TestGetBearerToken(t *testing.T) {
	headerWithAuth := http.Header{}
	headerWithAuth.Set("Authorization", "Bearer TOKEN_STRING")

	headerWithoutAuth := http.Header{}

	tests := []struct {
		name        string
		httpHeader  http.Header
		bearerValue string
		wantErr     bool
	}{
		{
			name:        "Bearer Token Exists",
			httpHeader:  headerWithAuth,
			bearerValue: "TOKEN_STRING",
			wantErr:     false,
		},
		{
			name:        "No Bearer Token",
			httpHeader:  headerWithoutAuth,
			bearerValue: "",
			wantErr:     true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			bearerValue, err := GetBearerToken(tt.httpHeader)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetBearerToken() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if bearerValue != tt.bearerValue {
				t.Errorf("GetBearerToken() bearerValue = %v, want %v", bearerValue, tt.bearerValue)
			}
		})
	}
}

func TestValidatePasswordStrength(t *testing.T) {
	tests := []struct {
		name        string
		password    string
		wantErr     bool
		matchString string
	}{
		{
			name:        "Valid Password",
			password:    "ValidPassword123!",
			wantErr:     false,
			matchString: "",
		},
		{
			name:        "Password without uppercase letter",
			password:    "password123!",
			wantErr:     true,
			matchString: "password must contain at least one capital letter",
		},
		{
			name:        "Password without lowercase letter",
			password:    "PASSWORD123!",
			wantErr:     true,
			matchString: "password must contain at least one lowercase letter",
		},
		{
			name:        "Password without number",
			password:    "Password!",
			wantErr:     true,
			matchString: "password must contain at least one number!",
		},
		{
			name:        "Password without special character",
			password:    "Password123",
			wantErr:     true,
			matchString: "password must contain at least one special character",
		},
		{
			name:        "Password with less than 8 characters",
			password:    "pass",
			wantErr:     true,
			matchString: "password must be at least 8 characters long",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidatePasswordStrength(tt.password)
			if (err != nil) != tt.wantErr {
				t.Errorf("ValidatePasswordStrength() error = %v, wantErr %v", err, tt.wantErr)
			}
			if err != nil && err.Error() != tt.matchString {
				t.Errorf("ValidatePasswordStrength() error = %v, wantErr %v", err, tt.matchString)
			}
		})
	}
}
